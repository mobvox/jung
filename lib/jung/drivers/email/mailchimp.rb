module Jung::Drivers::Email::Mailchimp

  attr_reader :gb, :list_id, :id
  attr_writer :gb, :list_id

  def self.extended(base)
    base.list_id = base.config.options[:drivers][:mailchimp][:list_id]
    base.gb = Gibbon.new base.config.options[:drivers][:mailchimp][:api_key]
  end
  ...
  def save
    ...
    @id = self.gb.campaign_create({ :type => :regular, :options => options, :content => content })
